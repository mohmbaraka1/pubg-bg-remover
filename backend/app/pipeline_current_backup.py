...
class BackgroundRemovalPipeline:
    ...
    def remove_background(self, image_bytes: bytes) -> bytes:
        ...
        # === فحص جودة تلقائي ===
        # لو مساحة القناع الناتج أصغر بشكل غير طبيعي من صندوق الكشف الأصلي
        # (مؤشر قوي على أجزاء اختفت بالغلط - شائع مع عناصر فاتحة/شفافة مثل
        # أسلحة جليدية أو ملابس بيضاء تتشابه مع الخلفية)، نعيد المعالجة
        # تلقائياً بعتبة أكثر تساهلاً بدل ما نسلّم نتيجة ناقصة مباشرة.
        ...
        if quality_ratio < 0.35:
            ...
            # نأخذ النتيجة الأكثر تساهلاً فقط لو فعلاً حسّنت المساحة بشكل ملموس
